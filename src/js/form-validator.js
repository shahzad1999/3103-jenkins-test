...
function Validator(formSelector) {
    ...
    var validatorRules = {
        required(value) {
            ...
            return value ? '' : `Please fill in empty field!`;
        },
        email(value) {
        ...
        max(max) {
            return (value) => {
                return value.length <= max ? '' : `Please enter at most ${max} characters!`;
            }
        },
        confirm(selector) {
            return (value) => {
                return value == document.querySelector(selector).value ? '' : `Re-entered password is incorrect!`;
            }
        },
        checkone(name) {
            return () => {
                var checked = formElement.querySelector(`[name="${name}"]:checked`);
                return checked ? '' : `Please select at least 1 option!`;
            }
        }
    };
